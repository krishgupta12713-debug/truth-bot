...
// Read token from env var first, then from config. Trim to remove stray whitespace.
const _token = process.env.BOT_TOKEN || config.botToken;
if (!_token) {
    console.error("No bot token provided. Set `BOT_TOKEN` environment variable or add `botToken` in config.js");
    process.exit(1);
}
